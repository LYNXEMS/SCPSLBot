...
class Patreon:
    ...
    async def on_member_update(self, before, after):
        ...
        if len(set(after.roles).intersection(patroles)) > 0 and discord.utils.get(message.server.roles, name="Patreon Supporter") not in after.roles:
            await self.bot.add_roles(member, discord.utils.get(server.roles, name="Patreon Supporter"))
        elif len(set(after.roles).intersection(patroles)) == 0 and discord.utils.get(message.server.roles, name="Patreon Supporter") in after.roles:
