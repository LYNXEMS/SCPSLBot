...
class Patreon:
    ...
    async def on_member_update(self, before, after):
        ...
        if len(list(set(after.roles).intersection(patroles))) > 0 and discord.utils.get(message.server.roles, name="Patreon Supporters") not in after.roles:
            await self.bot.add_roles(member, discord.utils.get(server.roles, name="Patreon Supporters"))
        elif len(list(set(after.roles).intersection(patroles))) == 0 and discord.utils.get(message.server.roles, name="Patreon Supporters") in after.roles:
